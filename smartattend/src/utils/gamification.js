...
/**
 * Enhanced leaderboard generation with ranking algorithm
 */
export function generateLeaderboardAndRank(allAttendance, allStudents) {
  const studentStats = {};
  ...
  allStudents.forEach(student => {
    studentStats[student._id] = {
      ...
      totalSessions: 0,
      attendedSessions: 0,
      totalPoints: student.gamificationStats?.totalPoints || 0,
      currentStreak: student.gamificationStats?.currentStreak || 0,
      achievements: student.gamificationStats?.achievements || [],
      lastAttendance: student.gamificationStats?.lastAttendance || null
    };
  });
  ...
  allAttendance.forEach(record => {
    if (studentStats[record.studentId]) {
      studentStats[record.studentId].attendedSessions++;
      
      // Add points from attendance record if available
      if (record.points) {
        studentStats[record.studentId].totalPoints += record.points;
      }
    }
  });
  ...
  const leaderboard = Object.values(studentStats).map(student => {
    ...
    const compositeScore = 
      (attendanceRate * 0.4) +           // 40% attendance rate
      (student.totalPoints * 0.3) +      // 30% total points
      (student.currentStreak * 5 * 0.2) + // 20% current streak (5 points per day)
      (student.achievements.length * 10 * 0.1); // 10% achievements (10 points each)

    return {
      ...student,
      attendanceRate: parseFloat(attendanceRate.toFixed(1)),
      compositeScore: parseFloat(compositeScore.toFixed(1)),
      badge: getBadgeFromPoints(student.totalPoints)
    };
  });
