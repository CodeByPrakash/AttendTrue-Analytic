...
export default function StudentDashboard({ user, authType }) {
  ...
  useEffect(() => {
    // Check if user is logged in via next-auth
    const isLoggedIn = nextAuthSession && nextAuthSession.user;
  ...
  // Display loading state if session is not loaded yet
  if (!nextAuthSession) {
    return <div>Loading user data...</div>;
  }

  // Determine which user object to use for display
  const displayUser = nextAuthSession?.user || user;
