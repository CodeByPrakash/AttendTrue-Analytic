  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        if (!["student", "teacher", "admin", "superadmin"].includes(credentials.role)) {
        ...
        try {
          ...
          // 3. Check if student is approved
          if (userDoc.role === 'student' && userDoc.approvalStatus !== 'approved') {
            throw new Error("Your account is pending admin approval. Please contact your administrator.");
          }
  ...
  callbacks: {
    ...
    async session({ session, token }) {
      if (token && session.user) {
        ...
        // Ensure image is either null or omitted to prevent serialization error
