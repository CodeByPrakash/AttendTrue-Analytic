...
export default async function handler(req, res) {
  ...
  try {
    ...
    const attendanceByCourse = Object.values(courseData).map(c => ({
        ...
        // 5. Status breakdown across attendance (prefer status, fallback to method)
        const statusMap = {};
        for (const att of allAttendanceForTeacher) {
            const key = att.status || att.method || 'unknown';
            statusMap[key] = (statusMap[key] || 0) + 1;
        }
        const statusBreakdown = Object.entries(statusMap).map(([name, count]) => ({ _id: name, count }));

        // 6. Identify at-risk students (e.g., < 75% attendance)
